...
public class BaseballControll : MonoBehaviour
{
    ...
    public void Pitched(bool isHitted)
    {
        ...
        {
            // 중간경로 계산 
            hitterPaths[hitterPaths.Length - 2].position = (hitterPaths[hitterPaths.Length - 1].position + hitterPaths[hitterPaths.Length - 3].position) * 0.7f + Vector3.up * midPosHeight;

            Debug.Log(hitterPaths[1].transform.position);

            iTween.MoveTo(gameObject, iTween.Hash("position", hitterPaths[0],
